...
| 适用范围 | MVP 到首版商业化上线的开发排期、功能拆分、验收闯关、风险补齐、当前实现核对 |
...
本节按 2026-06-09 当前仓库代码核对，参考范围包括 `package.json`、`apps/api`、`apps/worker`、`apps/web`、`packages/*`、`infra/*` 和 `tests/*`。这里讲的是“项目实际已经长出来什么”，不是文档里想象出来什么。说白了，代码不会吹水，文档会。

### 25.1 当前仓库实现概览

| 模块 | 当前实现 | 状态 | 证据 | 后续处理 |
| --- | --- | --- | --- | --- |
| Monorepo 与脚本 | 根工程已有 `apps/*`、`packages/*` workspace；提供 `dev:*`、`build`、`typecheck`、`test`、`smoke`、`load:smoke`、备份恢复和本地生产部署脚本 | 已落地 | `package.json`、`infra/scripts/*` | 增加 lint/format 脚本后再纳入强门禁 |
| API 服务 | Fastify API 已覆盖认证、用户、积分、生成、上传、图片、订单、支付 webhook、后台、指标、维护接口 | 已落地 | `apps/api/src/main.ts` | 后续拆分路由/服务层，避免单文件继续胀大 |
| Web 前端 | Next.js 页面已有首页、登录、注册、生成、历史、收藏、套餐、订单、账户、后台 | 已落地 | `apps/web/app/*` | 补图片详情页、任务详情页、复制提示词、再次生成、删除确认、真实支付跳转体验和移动端专项验收 |
| Worker | 支持 inline 轮询与 BullMQ worker，处理任务状态、调用 provider、存图、失败退款、超时退款 | 已落地 | `apps/worker/src/main.ts`、`packages/queue/src/index.ts` | 补并发控制、worker 优雅退出、任务锁和 provider 超时配置 |
| 数据层 | JSON Store 与 Prisma Store 双实现；Prisma schema 覆盖用户、会话、积分、任务、参考图、图片、收藏、套餐、订单、支付事件、安全事件、规则、审计日志 | 已落地 | `packages/database/src/index.ts`、`packages/database/prisma/schema.prisma` | 真正生产环境必须走 Prisma/PostgreSQL，不能继续依赖 JSON Store |
| 积分账本 | 注册赠送、生成扣减、失败退款、订单发放、后台调整、幂等键均已实现 | 已落地 | `spendCredits`、`grantCredits`、`adjustCredits`、测试用例 | 补积分过期、权益周期、运营手动冲正审批 |
| 图片生成 | 支持文生图、质量/比例/数量/风格、`clientRequestId` 幂等、参考图 ID、轮询结果 | 沙箱可用 | mock provider 生成 SVG | 接真实 AI provider，补真实模型错误码、成本映射和输出格式 |
| 参考图上传 | 支持 base64 上传、MIME 声明校验、文件签名校验、PNG/JPEG/WebP 尺寸读取、重复内容去重、TTL 字段 | 已落地 | `inspectReferenceUpload`、集成测试 | 前端仍是 base64 直传，生产应改对象存储直传或分片上传 |
| 图片资产 | 已支持图片列表、详情接口、收藏/取消、软删除、下载 URL、后台隐藏 | 基本落地 | `/api/images*`、`favorites` 页面 | 缩略图只是字段，缺真实缩略图生成；下载 URL 目前返回 `publicUrl`，生产应走短期签名 URL |
| 支付订单 | 支持套餐、创建订单、mock 支付、webhook 幂等、金额校验、重复回调防重、过期订单关闭、补偿对账 | 沙箱可用 | `packages/payments`、`/api/payments/webhooks/:provider`、集成测试 | 接 Stripe/Creem/Paddle 等真实 provider 前不得开放真实付费 |
| 内容安全 | 支持本地文本规则、后台安全规则、上传 MIME 检查、生成图 MIME 检查、安全事件记录 | 基本落地 | `packages/safety`、`safetyRules`、测试 | 缺第三方审核、人审队列、图片语义审核和申诉处理 |
| 管理后台 | 支持指标、用户搜索/状态、积分调整、任务/图片/订单筛选、套餐管理、安全规则、审计日志、维护对账 | 已落地 | `/api/admin/*`、`apps/web/app/admin/page.tsx` | 补后台详情页、操作二次确认、管理员不能误封自己 |
| 可观测性 | API 有 `requestId`、路由耗时指标、领域指标、告警快照、feature flags、健康检查 | 基本落地 | `/health`、`/api/admin/metrics` | 缺结构化日志落盘/采集、Prometheus/OpenTelemetry、告警通知通道 |
| 部署 | 已有开发 compose、生产风格 compose、Web/API/Worker Dockerfile、灰度 smoke/load 脚本、JSON 备份恢复脚本 | 基本落地 | `infra/*` | 缺 CI/CD、TLS/CDN、真实密钥注入、生产备份演练 |
| 测试 | 有领域单测和端到端 API/Worker 集成测试，覆盖生成、上传、权限、支付、后台、补偿、指标 | 已落地 | `tests/domain.test.mjs`、`tests/api-flow.test.mjs` | 缺前端组件/E2E、并发压测、真实 provider contract tests |

### 25.2 按闯关核对的当前状态

| 关卡 | 原计划目标 | 当前状态 | 判断 |
| --- | --- | --- | --- |
| 第 0 关：范围冻结与技术基线 | 范围、供应商、成本、环境基线 | 文档齐全，mock provider 和环境变量齐全 | 已过 MVP 基线 |
| 第 1 关：工程地基 | Monorepo、服务、数据库、质量门禁 | 工程、包、Docker、脚本已具备；lint 脚本暂缺 | 基本通过，补 lint 后更稳 |
| 第 2 关：账户与权限 | 注册登录、会话、角色、管理员权限 | API 与前端均已实现，后台接口做 admin 校验 | 已通过 |
| 第 3 关：积分与套餐基础 | 积分账户、流水、套餐、成本预估 | 已实现积分账户、流水、套餐、成本函数和报价接口 | 已通过 MVP，缺积分过期 |
| 第 4 关：图片生成任务闭环 | 创建任务、扣积分、队列、Worker、结果保存 | 已实现完整 mock 闭环和集成测试 | 沙箱通过，真实 provider 未过 |
| 第 5 关：任务补偿与状态体验 | 失败退款、超时扫描、状态轮询、重试 | API/Worker 和前端已覆盖，失败与重复退款有测试 | 已通过 MVP |
| 第 6 关：图片资产管理 | 历史、收藏、下载、删除、详情、缩略图 | 历史、收藏、下载签名、软删除、详情页、再次生成、缩略图和列表懒加载均已实现，并补充越权取消收藏回归测试 | 本地开发通过，真实对象存储/CDN 仍需上线前验收 |
| 第 7 关：订单与支付 | 下单、支付回调、权益发放、订单列表 | mock 支付、webhook、对账已实现 | 沙箱通过，真实支付未过 |
| 第 8 关：内容安全与上传 | Prompt 审核、参考图上传、生成图审核 | 本地规则和文件签名校验已实现 | MVP 可用，合规级未过 |
| 第 9 关：管理后台 | 用户、任务、图片、订单、套餐、审计 | API 和前端后台已覆盖主要操作 | 基本通过，缺更细操作防误触 |
| 第 10 关：性能、可观测性与部署 | 日志、指标、告警、缓存、CDN、部署 | 健康检查、指标、告警、Docker 已实现 | 部分通过，生产观测未过 |
| 第 11 关：总回归与灰度上线 | E2E、压测、故障演练、上线清单 | 有集成测试、smoke、load smoke、灰度 checklist | 部分通过，缺真实环境演练 |

### 25.3 未完成或未达到生产标准的补缺项
...
| P1 | 前端 E2E 与响应式验收 | 后端集成测试较完整，前端自动化不足 | Playwright 覆盖登录、生成、支付、后台、移动端关键流 | 第 11 关 |
| P1 | 并发与一致性压测 | JSON Store/update 和 Prisma write-all 模式不适合高并发 | 并发生成/支付压测通过，关键写路径改为数据库事务级更新 | 第 5/7/10 关 |
| P1 | CI/CD 与发布回滚 | 本地脚本存在，缺真实流水线 | CI 执行门禁，CD 支持环境变量注入、镜像版本、回滚和 smoke | 第 10/11 关 |
| P1 | 用户密码重置 | 需求和技术文档包含密码重置接口，当前 API/前端未实现 | 邮箱验证、重置 token TTL、限流、审计和前端流程完成 | 第 2 关 |
| P1 | 图片详情工作流补齐 | 已补独立图片详情页、复制提示词、回填参数再次生成、删除二次确认、积分和参数展示；历史/收藏页可进入详情 | 移动端真实浏览器 E2E 与视觉验收 | 第 6/11 关 |
| P1 | 管理后台查询能力补齐 | 需求要求按用户、状态、时间、订单号查询；当前后台缺时间范围和订单号搜索，图片定位信息也偏少 | 后台列表支持时间范围、订单号、用户、状态组合查询和详情抽屉 | 第 9 关 |
| P1 | 模型与成本配置后台 | 需求要求管理员配置模型参数，当前模型和成本映射在代码常量/provider 中 | 后台可配置默认模型、质量倍率、尺寸成本、provider 开关并有审计 | 第 3/9 关 |
| P1 | 订单与权益展示细化 | 需求要求账户页查看订单记录和权益状态，当前账户页主要展示积分与流水，订单在独立页 | 账户页展示当前权益、套餐有效期、最近订单入口和异常订单提示 | 第 7 关 |
| P2 | 图生图强度、变体、放大、局部重绘 | 参考图上传已做，但高级图像编辑能力未做 | 按第 27 章逐步实现，不挤进 MVP | 第 13 周以后 |
| P2 | 商用授权与版权声明 | 需求后续增强包含商用授权与版权声明管理，当前没有产品文案、授权记录和下载凭证 | 套餐权益明确商用范围，下载页/订单页可查看授权说明 | 第 13 周以后 |
| P2 | 提示词优化助手与模板市场 | 需求后续增强包含提示词优化、风格模板市场，当前未实现 | 做成独立增强模块，不阻塞 MVP；模板上架需审核和版本管理 | 第 17 周以后 |
| P2 | 批量生成与提示词变量 | 当前 `maxQuantity=4`，无批量任务语法 | 支持变量展开预览、批量扣费、子任务队列、部分失败恢复 | 第 17 周以后 |

### 25.4 当前上线判断

| 环境 | 判断 | 原因 |
| --- | --- | --- |
| 本地开发演示 | 可以 | mock AI、mock 支付、inline 存储、JSON Store 已能跑通完整体验 |
| 内部灰度演示 | 可以，但必须标注 mock | 可以验证产品流程、后台、指标和补偿逻辑，不能验证真实成本和供应商稳定性 |
| 收费公测 | 不建议 | 真实支付、真实 AI、真实对象存储、生产观测和前端 E2E 仍未完成 |
| 商业化上线 | 不可上线 | P0 生产依赖未接入，当前上线等于拿 mock 当真钱包，靓仔都救不了这种事故 |

当前最务实的补缺顺序是：先把真实 AI provider、对象存储、支付 provider、下载签名、缩略图和生产数据路径打通；再补前端详情页、前端 E2E、真实观测和 CI/CD。不要先冲 P1/P2 炫技功能，否则会变成“图生图很酷，但用户付完钱不到账”的大型翻车现场。

### 25.5 需求文档和技术文档差异补充

以下是从 `AI图片生成网站用户需求文档.md` 与 `Imagora技术实现文档.md` 再对照代码后补出来的差异。它们未必都阻塞 MVP 演示，但会影响正式验收、收费、公测或合规说明。

| 来源 | 文档承诺 | 当前项目状态 | 处理结论 |
| --- | --- | --- | --- |
| 用户需求 FR-007/FR-008 | 用户可查看资料、修改昵称和头像 | API 已有 `PATCH /api/users/me`，账户页偏展示，编辑体验不完整 | P1 补账户编辑表单和头像 URL 校验体验 |
| 用户需求 FR-009 | 用户可重置密码 | 当前无密码重置 API、邮件或前端页面 | P1 补邮箱验证、token TTL、限流和审计 |
| 用户需求 FR-024 | 下载文件可正常打开，文件名可识别 | API 返回 URL，前端缺明确下载按钮/文件名策略；inline data URL 不代表生产下载 | P0/P1 结合对象存储签名 URL 和前端下载体验补齐 |
| 用户需求 FR-026 | 删除前二次确认 | API 支持软删除，前端未形成完整二次确认流程 | P1 补确认弹层和撤销/误删说明 |
| 用户需求 FR-027/FR-028 | 复制提示词、基于历史再次生成 | 当前历史页展示列表，缺一键复制和回填再生成 | P1 补图片详情/历史卡片操作 |
| 用户需求 FR-049/FR-053 | 后台按用户、状态、时间、订单号查询 | API 有用户/status 过滤，缺时间范围和订单号搜索 | P1 扩展查询参数和后台筛选控件 |
| 用户需求 FR-054 | 管理员配置模型参数 | 当前模型 provider 和成本映射写在代码/环境变量中 | P1 做模型成本配置表和后台审计 |
| 用户需求后续增强 | 商用授权与版权声明管理 | 当前没有授权文案、下载凭证和套餐权益说明 | P2 做授权说明，不要混进 MVP 主链路 |
| 技术文档 7.2 | 密码重置接口 | 未实现 | P1 补接口和前端流程 |
| 技术文档 7.7 | `/api/admin/users/:userId` 用户详情 | 当前只有用户列表和状态/积分调整接口 | P1 补后台用户详情接口或详情抽屉 |
| 技术文档 8.1 | `image-postprocess`、`payment-compensation`、`credit-refund`、`cleanup` 队列 | 当前主要只有 generation 队列，支付补偿在维护接口中同步执行 | P1/P2 按风险逐步拆出独立队列 |
| 技术文档 11.1 | HttpOnly/Secure/SameSite Cookie | 当前是 bearer token + `localStorage` | P0 上线前升级会话安全 |
| 技术文档 11.4 | 支付回调签名校验 | mock provider 未做真实签名校验 | P0 接真实支付时必须完成 |
| 技术文档 13/14 | CDN、结构化日志、外部告警 | 当前只有 Docker、内存指标和后台告警快照 | P1 补生产可观测性与 CDN/对象存储链路 |
